...
/****** Object:  View [Master].[Dim_Well_Header_SigmaFlow]    Script Date: 10/31/2024 1:12:55 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER VIEW [Master].[Dim_Well_Header_SigmaFlow]
AS
...
---2
SELECT
      ROW_NUMBER() OVER (PARTITION BY SEQNUM ORDER BY SEQNUM) AS [Unique SEQNUM], * 
FROM (
---1
SELECT 
       ROW_NUMBER() OVER (PARTITION BY Well_Name_Antero_Standard ORDER BY Well_Name_Antero_Standard) AS [Unique Well Name]
      ,[Sources]
      ,[Sources Rownum]
      ,[Sources Rank]
      ,[Sources Length]
...
WHERE 
Well_Name_Antero_Standard IS NOT NULL 
AND 
Sources LIKE '%G%'
----1
) aa
WHERE [Unique Well Name] = 1
---2
) bb
WHERE [Unique SEQNUM] = 1
---3
---ORDER BY 1 DESC
