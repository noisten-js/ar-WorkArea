...
from EDW.edw_references import SqlServer
...
def do_the_thing():
    ...
    dst_cn = db_connect('testdw01', 'EDW_Stage')
    dst_cr = dst_cn.cursor()

    tbl_sql = (
        f"SELECT\n"
        f"   ObjectId = t1.object_id\n"
        f"  ,SchemaName = t0.name\n"
        f"  ,TableName = t1.name\n"
        f"FROM\n"
        f"  sys.schemas t0\n"
        f"  INNER JOIN sys.objects t1\n"
        f"      ON t0.schema_id = t1.schema_id\n"
        f"WHERE 1=1\n"
        f"    AND t0.name = 'Stage'"
        r"    AND t1.name LIKE 'Enertia\_%' ESCAPE '\'"
    )
    ...
    non_key_sql = (
        ...
        "       FROM\n"
        "           sys.indexes i0\n"
        "           INNER JOIN sys.index_columns i1\n"
        "               ON i0.object_id = i1.object_id\n"
        "               AND i0.index_id = i1.index_id\n"
        "               AND i0.is_primary_key = 1\n"
        "       WHERE 1=1\n"
        "           AND t1.object_id = i0.object_id\n"
        "           AND t2.column_id = i1.column_id\n"
        "    )\n"
    )

    tbl_data = dst_cr.execute(tbl_sql).fetchall()
    key_data = dst_cr.execute(key_sql).fetchall()
    non_key_data = dst_cr.execute(non_key_sql).fetchall()

    return tbl_data, key_data, non_key_data


def ut():
    cn = db_connect('testdw01', 'EDW')
    cr = cn.cursor()
    sql = ("select _rowhash=NULL, * from sys.columns")
    #     "SELECT"
    #     "    _rowhash = NULL"
    #     "   ,Type"
    #     "   ,Well_Name"
    ...
    #     "   ,Surface_Location_Shape			= Surface_Location_Shape.STAsBinary()"
    #     "   ,Surface_Location_Shape_GIS		= Surface_Location_Shape_GIS.STAsBinary()"
    #     "   ,Wellbore_Shape_GIS				= Wellbore_Shape_GIS.STAsBinary()"
    #     "   ,Wellbore_Shape_NAD27_BLM_GIS	= Wellbore_Shape_NAD27_BLM_GIS.STAsBinary()"
    #     "   ,BottomHole_Location_Shape_GIS	= BottomHole_Location_Shape_GIS.STAsBinary()"
    #     "   ,EDW_Create_Date"
    #     "   ,Hub_Well_Header_ID"
    ...
    #     "FROM"
    #     "   Master.Dim_Well_Header"
    # )
    data = cr.execute(sql).fetchall()
    for r in data:
        rdata = [v for v in r[1::]]
        r._rowhash = hash_objects(rdata)
